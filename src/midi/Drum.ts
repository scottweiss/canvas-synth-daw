...
export class Drum {
    ...
    constructor(frequency: number, type: OscillatorType, attackTime: number = 0.1, releaseTime: number = 2) {
    ...
    play() {
        ...
        if (!this.isPlaying) {
          ...
          this.gainNode.gain.linearRampToValueAtTime( 0.9, this.audioEngine.audioContext.currentTime + this.attackTime);
          this.gainNode.gain.linearRampToValueAtTime( 0, this.audioEngine.audioContext.currentTime + this.attackTime + this.releaseTime);
        console.log(this)
    }
}
